...
int udpOpen(ClientData , Tcl_Interp *, int , Tcl_Obj *const objv[]);
int udpConf(ClientData , Tcl_Interp *, int , Tcl_Obj *const objv[]);
int udpPeek(ClientData , Tcl_Interp *, int , Tcl_Obj *const objv[]);
...
static int hasOption(int argc, char argv[8][42],const char* option );
 ...
 */
static int
hasOption(int argc, char argv[8][42],const char* option ) 
{
	int i;
	for (i=0;i<argc;i++) {
 ...
 * CopyTclArgv --
 *
 * Copy strings from Tcl_Obj to argv used by most of the code.
 * ----------------------------------------------------------------------
 */

void CopyTclArgv(Tcl_Obj *const objv[], int argc, char argv[8][42])
{
    int idx;
    for(idx=0;(idx<argc) && (idx<8); idx++) {
        strncpy(argv[idx], Tcl_GetString(objv[idx]), 40);
		argv[idx][40] = '\0';
    }
}
/*
			 ...
			 CONST84 char *optionName, Tcl_DString *optionValue)
{
	...
	if (optionName == NULL) {
		...
		}else if (!strcmp("-localaddr", optionName)) {
            if (statePtr->localaddr && *statePtr->localaddr) {
				Tcl_DStringSetLength(&dsInt, TCL_INTEGER_SPACE);
				sprintf(Tcl_DStringValue(&dsInt), "%u", 
					ntohs(statePtr->localport));
				Tcl_DStringAppendElement(&ds, statePtr->localaddr);
				Tcl_DStringAppendElement(&ds, Tcl_DStringValue(&dsInt));
			}	
